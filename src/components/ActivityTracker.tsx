...
const ActivityTracker: React.FC<ActivityTrackerProps> = ({ onStopTracking, challengeGoalSteps }) => {
  ...
  const resetTrackerStates = useCallback((keepActivitySettings = false) => {
    if (!keepActivitySettings) { // Only reset if not just a pause/resume cycle
      setIsTracking(false);
  ...
  const handlePauseResume = () => {
    ...
    if (!isPaused && isTracking) { // Resuming
      if (!startTime) { // If starting from a paused state where it never truly started
  ...
  const handleStop = () => {
    ...
    if (!challengeGoalSteps && steps > 0) { // Only show generic reward for generic activities with some effort
  ...
  return (
    <>
      <div className="flex flex-col items-center space-y-6 p-4 bg-background text-foreground animate-fade-in-up">
        {challengeGoalSteps && (
          <Card className="w-full max-w-md bg-primary/10 border-primary mb-4">
        ...
        <div className="flex space-x-4 w-full max-w-md">
          {!isTracking && !activityCompleted ? ( // Show Start if not tracking and not completed
          ...
          ) : isTracking ? ( // Show Pause/Resume if actively tracking
              <Button 
                onClick={handlePauseResume} 
                variant="outline" 
                className="flex-1 border-pink-500 text-pink-500 hover:bg-pink-500 hover:text-primary-foreground"
              >
                  {isPaused ? <Play size={20} className="mr-2" /> : <Pause size={20} className="mr-2" />}
                  {isPaused ? 'Resume' : 'Pause'}
              </Button>
          ) : null /* Activity completed or never started, other buttons handle this */}
          
          {/* Restart button always available if tracking has started or completed, but not if it's currently active & unpaused */}
          {(startTime || activityCompleted) && (isPaused || !isTracking) && (
            <Button 
              onClick={handleStart} // Restart is essentially a new start
              variant="outline" 
      ...
      {showGenericRewardCard && !challengeGoalSteps && ( // Only show generic reward if not a challenge
        <ActivityRewardCard 
          coinsEarned={coinsEarned} // coins from steps
