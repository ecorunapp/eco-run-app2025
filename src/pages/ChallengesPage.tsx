...
const ChallengesPage: React.FC = () => {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-background text-foreground">
      ...
      <main className="flex-grow p-4 space-y-8 pb-20"> {/* Added pb-20 for BottomNav */}
        
        {/* Walk Challenges Section */}
        <section>
          <h2 className="text-xl font-semibold mb-4 text-foreground">Walk Challenges</h2>
          {walkChallenges.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {walkChallenges.map(challenge => (
                <ChallengeCard key={challenge.id} challenge={challenge} />
              ))}
            </div>
          ) : (
            <p className="text-muted-foreground">No walking challenges available right now.</p>
          )}
        </section>
        ...
        <section>
          ...
          {runChallenges.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {runChallenges.map(challenge => (
                <ChallengeCard key={challenge.id} challenge={challenge} />
              ))}
            </div>
          ) : (
            <p className="text-muted-foreground">No running challenges available right now.</p>
          )}
        </section>
