...
import { Settings, User, LogOut, Edit3, CreditCard, Bell, ShieldCheck, Palette, Mail, ChevronRight, Weight, Zap as HeightIcon, Loader2, TrendingUp, Star, MapPin, Coins, ShoppingBag } from '@/components/icons';
...
const ProfilePage: React.FC = () => {
  ...
  // Placeholder user data - will be overridden by fetched profile or skeletons
  ...
  const displayUser = profile ? {
    ...
    email: profile.username || authUser?.email || 'N/A',
  ...
  const profileMenuItems = [
    { title: 'Edit Profile', icon: Edit3, action: () => navigate('/edit-profile') },
    { title: 'Order Ecotab Card', icon: ShoppingBag, action: () => navigate('/order-ecotab') },
    { title: 'Notification Settings', icon: Bell, action: () => navigate('/notification-settings') },
    { title: 'Privacy & Security', icon: ShieldCheck, action: () => console.log('Privacy clicked') },
    { title: 'Appearance', icon: Palette, action: () => navigate('/appearance-settings') },
  ];
  ...
  const showSkeletons = profileLoading && !profile;
  ...
  return (
    <div className="flex flex-col min-h-screen bg-eco-dark text-eco-light">
      <header className="p-4 flex justify-between items-center sticky top-0 bg-eco-dark/80 backdrop-blur-md z-40 shadow-sm">
        ...
        <Button variant="ghost" size="icon" className="text-eco-gray hover:text-eco-accent" onClick={() => navigate('/settings')}>
          <Settings size={24} />
        </Button>
      </header>
      ...
      <main className="flex-grow p-4 space-y-6 overflow-y-auto pb-24 animate-fade-in-up">
          ...
          <CardContent className="pt-6 flex flex-col items-center text-center">
            {showSkeletons ? (
              <>
                ...
                <Skeleton className="h-4 w-32 mt-1 mb-3" />
        ...
        <Card className="bg-eco-dark-secondary border-transparent shadow-lg">
          ...
          <CardContent className="grid grid-cols-2 sm:grid-cols-3 gap-4 text-center">
            {showSkeletons ? (
              <>
                {[...Array(3)].map((_, i) => (
                  <div key={i} className="p-3 bg-eco-dark rounded-lg flex flex-col items-center">
        ...
        <Card className="bg-eco-dark-secondary border-transparent shadow-lg">
          ...
          <CardContent className="space-y-1">
            {showSkeletons ? (
              <>
                {[...Array(profileMenuItems.length)].map((_, i) => (
