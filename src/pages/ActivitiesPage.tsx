...
const ActivitiesPage: React.FC = () => {
  ...
  const activeChallenge = location.state?.activeChallenge as (Challenge & {
  ...
  return <div className="flex flex-col min-h-screen bg-background text-foreground">
      <header className="p-4 sticky top-0 z-10 shadow-sm border-b bg-slate-900 px-[7px] py-[12px] my-0 mx-0">
        <h1 className="text-2xl font-bold text-center text-primary">
          {activeChallenge ? `Challenge: ${activeChallenge.title}` : "Track Activity"}
        </h1>
      </header>
      <main className="flex-grow p-0 sm:p-4 overflow-y-auto pb-20 bg-gray-900"> {/* pb-20 for BottomNav */}
        <ActivityTracker onStopTracking={handleStopTracking} activeChallenge={activeChallenge} // Pass activeChallenge here
      />
      </main>
      <BottomNav />
    </div>;
